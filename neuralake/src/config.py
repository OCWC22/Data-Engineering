...
class NeuralakeConfig:
    ...
    def _validate_local_config(self) -> None:
        ...
        if self.s3.allow_http:
            warnings.warn(
                "HTTP connections enabled - only use for local development",
                stacklevel=2,
            )
        if self.s3.allow_unsafe_rename:
            warnings.warn(
                "Unsafe S3 rename enabled - only use for local development",
                stacklevel=2,
            )
    ...
    def _log_configuration_summary(self) -> None:
        ...
        logger.info(
            f"Neuralake configuration loaded for environment: {self.environment.value}"
        )
        ...
        logger.info(
            f"Data quality validation: {'enabled' if self.data_quality.enforce_schema else 'disabled'}"
        )
        logger.info(
            f"Security features: encryption={'enabled' if self.security.encrypt_at_rest else 'disabled'}"
        )

    @classmethod
    def from_environment(cls, env_name: str | None = None) -> "NeuralakeConfig":
    ...
    @classmethod
    def from_file(cls, config_path: str | Path) -> "NeuralakeConfig":
        ...
        if environment == Environment.LOCAL:
            return cls(
                ...
                security=SecurityConfig(
                    encrypt_at_rest=False, encrypt_in_transit=False, audit_enabled=False
                ),
            ...
            return cls(
                environment=environment,
                s3=S3Config(
                    region="us-west-2", allow_http=False, allow_unsafe_rename=False
                ),
                data_quality=DataQualityConfig(
                    enforce_schema=True, validate_on_write=True
                ),
                logging=LoggingConfig(level=LogLevel.INFO),
                security=SecurityConfig(
                    encrypt_at_rest=True, encrypt_in_transit=True, audit_enabled=True
                ),
            ...
            return cls(
                environment=environment,
                s3=S3Config(region="us-west-2", max_connections=200),
                data_quality=DataQualityConfig(
                    enforce_schema=True,
                    validate_on_write=True,
                    alert_on_quality_degradation=True,
                ),
                logging=LoggingConfig(level=LogLevel.INFO, enable_json_logging=True),
                security=SecurityConfig(
            ...
            return cls(
                environment=environment,
                s3=S3Config(region="us-west-2", max_connections=500, max_retries=5),
                data_quality=DataQualityConfig(
                    enforce_schema=True,
                    validate_on_write=True,
                ...
                logging=LoggingConfig(
                    level=LogLevel.WARNING, enable_json_logging=True, async_logging=True
                ),
                security=SecurityConfig(
                    encrypt_at_rest=True,
                    encrypt_in_transit=True,
                ...
                performance=PerformanceConfig(
                    enable_query_cache=True, max_cache_size_mb=4096, async_io=True
                ),
    ...
    def _apply_environment_overrides(
        ...
        if os.getenv("NEURALAKE_LOG_LEVEL"):
            try:
                config.logging.level = LogLevel(
                    os.getenv("NEURALAKE_LOG_LEVEL").upper()
                )
            except ValueError:
                warnings.warn(
                    f"Invalid log level: {os.getenv('NEURALAKE_LOG_LEVEL')}",
                    stacklevel=2,
                )
        ...
        if os.getenv("NEURALAKE_MAX_WORKERS"):
            try:
                config.performance.max_worker_threads = int(
                    os.getenv("NEURALAKE_MAX_WORKERS")
                )
            except ValueError:
                warnings.warn(
                    f"Invalid max workers value: {os.getenv('NEURALAKE_MAX_WORKERS')}",
                    stacklevel=2,
                )
...
_config: NeuralakeConfig | None = None
