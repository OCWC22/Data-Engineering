...
def parse_docstring(docstring):
    ...
    for i, line in enumerate(lines):
        ...
        is_ascii_art = (
            ...
            line.startswith('shape:') or
            re.match(r'^\s*[│┌┐└┘├┤┬┴┼─═║╔╗╚╝╠╣╦╩╬]', original_line) or
        ...
        if current_section == "parameter" and line and not line.startswith('Parameters') and not line.startswith('Returns') and not in_code_block:
            if current_param:
                current_param['description'].append(line)
            continue
        
        # Check for section headers
        if line.lower() in ['parameters', 'parameters:', 'args:', 'arguments:'] and not in_code_block:
            # End any open code block
            if in_code_block and current_section == "description":
                description_lines.append("```")
                description_lines.append("")
    ...
    try:
        if inspect.isclass(obj):
            ...
            public_methods = [
                ...
                if not method_name.startswith('_') or method_name in ['__call__', '__str__', '__repr__']
            ]
